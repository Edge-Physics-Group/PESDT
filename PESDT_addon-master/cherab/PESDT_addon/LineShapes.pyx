...
cdef class OpaqueGaussianLine(LineShapeModel):
    ...
    def __init__(self,
        ...
        super().__init__(line, wavelength,
                         target_species,
                         plasma,
                         atomic_data)
...
cdef class OpaqueDeltaLine(LineShapeModel):
    ...
    Instead of adding a broadened profile, all radiance is deposited into
    the single spectral bin.
    ...
    cpdef OpaqueSpectrum add_line(self,
        ...
        spectrum.absorbances_mv[0] += absorbance*ds
        spectrum.samples_mv[0] += radiance*expl(-spectrum.absorbances_mv[0])
        return spectrum
