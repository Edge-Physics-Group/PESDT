...
def get_JETdefs(plot_defs = False, pulse_ref = 90531):
    ...
    pesdt_home = os.environ.get('PESDT_HOME', os.path.join(os.path.expanduser('~'), "PESDT/"))
    ...
    origin = [3.7438, 3.354]
    width = 0.095
    p2 = np.array([[3.7438, -2.0]])
    kg1_los = np.zeros((len(p2), 3, 2))
    for i in range(len(p2)):
        kg1_los[i, 0] = origin
        kg1_los[i, 1] = p2[i]
        kg1_los[i, 2] = [0, width]
    los_dict = {}
    los_dict['p1'] = kg1_los[:,0]
    ...
    angle = 58.97442 # Raysect wants fov in deg
