...
cdef class PESDTOpaqueMaxwellian(DistributionFunction):
    ...
    def __init__(self, object density, object temperature, object velocity, object emission, object absorbance, double atomic_mass):
        ...
        self._emission_dict = emission
        self._emission = autowrap_function3d(self._emission_dict[next(iter(self._emission_dict))])
        self._absorbance_dict = absorbance
        self._absorbance = autowrap_function3d(self._absorbance_dict[next(iter(self._absorbance_dict))])
        self._atomic_mass = atomic_mass
    ...
    cpdef double absorbance(self, double x, double y, double z) except? -1e999:
        ...
        return self._absorbance.evaluate(x, y, z)

    cpdef void update_absorbance(self, transition):
        """
        Updates the emission to the current transition
        """
        self._absorbance = autowrap_function3d(self._absorbance_dict[transition])